...
namespace Step.Interpreter.Steps
{
    internal class AssignmentStep : Step
    {
        ...
        public override bool Try(TextBuffer output, BindingEnvironment e, Task.Continuation k,
            MethodCallFrame? predecessor)
        {
            ...
            if (e.Unify(e.Resolve(LocalVariable), expValue, out BindingEnvironment result))
                return Continue(output, result, k, predecessor);
